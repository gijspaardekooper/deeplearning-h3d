...
class DeepSDFNetwork(nn.Module):
    def __init__(
        ...
        if weights:
            dict_weights = torch.load(weights)['model_state_dict']
            dict_weights = OrderedDict((k.replace('module.','') if ('module.' in k) else k, v) for k, v in dict_weights.items())
            self.load_state_dict(dict_weights)
